library(tidyverse)
library(cfbscrapR)
library(ggmap)
library(ggridges)
library(ggrepel)
library(patchwork)



years <- 2000:2020

year_df <- as.data.frame(years)
colnames(year_df) <- "year"

recruiting_info <- year_df %>%
  mutate(
    rec_info = purrr::map(year,cfb_recruiting_player)
  )

recruiting_raw <- recruiting_info %>% tidyr::unnest() %>%
  mutate(
    blue_chip = if_else(stars>=4,T,F)
  )


### State Card 
### Plot (1) State with locations on where primary blue chips come from 
### Plot (2) Recruiting %, what schools dominate (top 5 or so)
### 

## work on making it cleaner
# state_viz <- function(raw_recruit_df,state_abbr,min_year,max_year){
#   stopifnot()
#   state_yrs <- recruiting_raw %>% filter(state_province=="TX",year>min_year)
#   
# }


texas_3_yrs <- recruiting_raw %>% filter(state_province=="TX",year>2017)

city_blue_chip <- texas_3_yrs %>% group_by(city) %>%
  summarize(blue_cnt = sum(blue_chip)) %>%
  mutate(adr = paste0(city,", TX"),
         lat_long = geocode(adr))

city_deets <- city_blue_chip %>% select(city,lat_long) 
city_deets <- bind_cols(city_deets[1], reduce(city_deets[-1], tibble))
saveRDS(city_deets,"texas_lat_long.RDS")

texas_3_yrs <- texas_3_yrs %>% left_join(city_deets)

blue_chip_texas <- texas_3_yrs %>% filter(blue_chip)
  
m = map_data('state', region = 'Texas')

## add state map

test1 = ggplot() + 
  geom_polygon( data=m, aes(x=long, y=lat,group=group),colour="black", fill="white" ) + 
  stat_bin_hex(data=blue_chip_texas,aes(x=lon,y=lat)) + 
  scale_fill_viridis_c(
    option="B",
  ) +  
  labs(title = "Where do blue chip recruits come from?") + 
  theme_void(base_size = 16) 

## top 5 blue chip 

blue_chip_schools <- blue_chip_texas %>% group_by(committed_to) %>%
  summarise(cnt = n()) %>% ungroup() %>% 
  mutate(pct = cnt/sum(cnt),
         rank = rank(desc(pct))) %>% filter(!is.na(committed_to)) %>% arrange(pct) %>%
  mutate(
    team_names = fct_inorder(committed_to)
  ) %>% filter(rank <= 10)


## total talent

schools <- texas_3_yrs %>% group_by(committed_to) %>%
  summarise(cnt = n()) %>% ungroup() %>% 
  mutate(pct = cnt/sum(cnt),
         rank = rank(pct)) %>% filter(!is.na(committed_to)) %>% arrange(pct) %>%
  mutate(
    team_names = fct_inorder(committed_to)
  )

colnames(schools)[2:3] <- paste0("all_",colnames(schools)[2:3])

blue_chip_schoolsv2 <- blue_chip_schools %>% left_join(schools %>% select(-rank,-committed_to))

test2 = ggplot() +
  # remove axes and superfluous grids
  theme_classic(base_size = 16) +
  theme(
    axis.title = element_blank(),
    axis.ticks.y = element_blank(),
    axis.line = element_blank()
  ) +
  # add a dummy point for scaling purposes
  geom_point(data=blue_chip_schoolsv2,aes(x = 12, y = team_names),
              size = 0, col = "white") +
  # add the horizontal discipline lines
  geom_hline(yintercept = 1:nrow(blue_chip_schoolsv2), col = "grey80") +
  # add a point for each male success rate
  geom_point(data = blue_chip_schoolsv2,
             aes(x = cnt, y = team_names),
             size = 11,
             col = "#9DBEBB")  +
  geom_point(data = blue_chip_schoolsv2,
             aes(x = all_cnt, y = team_names),
             size = 11, col = "#468189") +
  geom_text(data= blue_chip_schoolsv2,
            aes(
              x = cnt, y = team_names,
              label=cnt), 
  col = "black") + 
  geom_text(data= blue_chip_schoolsv2,
            aes(
              x = all_cnt, y = team_names,
              label=all_cnt), 
            col = "black") + 
  geom_text(aes(x = x, y = y, label = label, col = label),
            data.frame(x = c(51 - 15, 51 + 0.6), y = nrow(blue_chip_schoolsv2) + 1, 
                       label = c("Blue Chip", "Total")), size = 6) +
scale_color_manual(values = c("#9DBEBB", "#468189"), guide = "none") +
  scale_y_discrete(expand = c(0.2, 0)) +
labs(title = "Where do recruits go?") 


team_info <- cfb_team_info()

logos <- team_info %>% unnest_wider(logos,names_sep = "_") %>% select(school,logo = logos_1)

rec_plot_data <- texas_3_yrs %>%
  left_join(logos, by = c("committed_to" = "school")) %>%
  mutate(position = case_when(position == "DUAL" ~ "QB",
                              position == "PRO" ~ "QB",
                              position == "OT" ~ "OL",
                              position == "OG" ~ "OL",
                              position == "OC" ~ "OL",
                              position == "LS" ~ "OL",
                              position == "APB" ~ "RB",
                              position == "FB" ~ "RB",
                              position == "OLB" ~ "LB",
                              position == "ILB" ~ "LB",
                              position == "WDE" ~ "DE",
                              position == "SDE" ~ "DE",
                              position == "P" ~ "P/K",
                              position == "K" ~ "P/K",
                              TRUE ~ position),
         position = factor(position,
                           levels = c("QB","RB","WR","TE",
                                      "ATH","OL","DE","DT",
                                      "LB","CB","S","P/K")))

max <- rec_plot_data %>% group_by(year1,position) %>% filter(rating == max(rating))


test3 = rec_plot_data %>%
  ggplot(aes(x = rating,y = reorder(position, desc(position)),fill=position)) +
  facet_wrap(~year1) + 
  geom_density_ridges(jittered_points = TRUE,alpha=0.7)  + 
  #geom_text_repel()
  #geom_density_ridges() + 
  #geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  #scale_fill_viridis_c(option = "C") + 
  #theme_bw(base_size=16) + 
  theme_ridges( center_axis_labels = TRUE) + 
  scale_fill_discrete(guide=FALSE) + 
  ggimage::geom_image(data = max,
                      aes(image = logo),
                      asp = 1.618 , size = .07) + 
  labs(y="",x="247 Sports Composite ")



## combine final plot
p = test1 | (test2 / test3) 
  
p + plot_annotation(title = 'Texas Recruiting Profile', 
                                          caption = '@msubbaiah1',
                                          theme = theme(plot.title = element_text(size = 20)))



